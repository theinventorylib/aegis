...
func (p *Plugin) BeginAuth(w http.ResponseWriter, r *http.Request, providerName string) error {
	...
	// Redirect to provider
	// The authURL is generated by Goth's OAuth provider (not user input) and always points
	// to a pre-configured provider endpoint (Google, GitHub, etc.). gosec cannot detect this.
	//nolint:gosec
