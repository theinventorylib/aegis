...
func TestSMSPlugin_Register(t *testing.T) {
	...
	// Mock DB ExecFunc to handle account creation and phone update
	...
	req := httptest.NewRequest("POST", "/api/sms/register", bytes.NewBuffer(body))
	...
	user, err := plugin.GetUserByPhone(context.Background(), "+1234567890")
	if err != nil {
